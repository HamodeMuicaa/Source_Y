...
def dbb():
    global db
    db = {}
    LOGGER("ميــوزك بحر").info(f"تم تحديث قاعدة بيانات البوت ...✓")
...
async def sudo():
    ...
    if sudoers:
        for user_id in sudoers:
            SUDOERS.add(user_id)
    LOGGER("ميــوزك بحر").info(f" تم تحميل قائمة مطورين البوت ...✓")
    ...
    if is_heroku:
        if config.HEROKU_API_KEY and config.HEROKU_APP_NAME:
            try:
                ...
                LOGGER("ميــوزك بحر").info(f"تم إضافة فارات البوت ...✓")
